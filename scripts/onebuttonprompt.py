...
from csv_reader import load_config_csv
from main import (build_dynamic_prompt, createpromptvariant, generateimages,
                  tryinterrupt)
from model_lists import (get_models, get_samplers, get_samplers_for_img2img,
                         get_upscalers, get_upscalers_for_img2img)
...
for filename in os.listdir(userfilesfolder):
    if filename.endswith(".csv") and filename.startswith("personal_artists") and \
            filename != "personal_artists_sample.csv":
...
for item in config:
    ...
    match item[0]:
        # objects
        case 'subject_vehicle':
            generatevehicle = False
        case 'subject_object':
            generateobject = False
        case 'subject_food':
            generatefood = False
        case 'subject_building':
...
if generatemanwoman or generatemanwomanrelation or generatefictionalcharacter or generatenonfictionalcharacter or \
        generatehumanoids or generatejob:
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        def gen_prompt(
            ...
            for i in range(5):
                promptlist.append(build_dynamic_prompt(
                    insanitylevel,
                    subject,
                    artist,
                    imagetype,
...
This generator will generate a complete full prompt for you and generate the image,
based on randomness. You can increase the slider, to include more things to put into the prompt.
Recommended is keeping it around 3-7. Use 10 at your own risk.

There are a lot of special things build in, based on various research papers.
Just try it, and let it surprise you.

Add additional prompting to the prefix, suffix and negative prompt in this screen.
The actual prompt fields are ignored.
</font>

### Subject Types

<font size="2">
You can choose a certain subject type, if you want to generate something more specific.
...
3. humanoid - A random humanoid, males, females, fantasy types, fictional and non-fictional characters. \
Can add clothing, features and a bunch of other things.
...
It can also generate a line from a poem or a song.

After choosing object, humanoid or concept a subselection menu will show.
You can pick further details here. When choosing humanoid, you can also select the gender you wish to generate.
...
Automatically, it will select between 0-3 artists out of 3483 artists for your prompt.
...
1. Normal
    ...
    8. portrait
    9. anime key visual
    10. only other types --> Will pick only from the more unique types, such as stained glass window or a funko pop

2. All modes below are considered a special image type mode.
    11. only templates mode --> Will only choose from a set of wildcarded prompt templates. \
    Templates have been gathered from various sources, such as CivitAI, prompthero, promptbook, etc. \
    only templates mode is perfect for beginners, who want to see some good results fast.
    12. art blaster mode --> art movements, stylings and artists.
    13. quality vomit mode --> qualifiers and stylings.
    14. color cannon mode --> color scheme's and moods.
    15. unique art mode --> other image types, art movements, stylings and lighting.
    16. massive madness mode --> Special generation mode, creates prompt soup. Almost pure randomness.
    17. photo fantasy mode --> photographs, cameras, lenses and lighting.
    18. subject only mode --> Will only generate a subject, with no additional frills.
    19. fixed styles mode --> Generate a subject on top of a fixed style.

### One in X chance to use special image type mode

<font size="2">
This controls how often it will pick a special generation mode. It is a 1 in X chance.
So lower means more often. This will only be applied of "type of image" is set to "all" and
there is no Overwrite type of image set.

When set to 1, it will always pick a random special generation mode.
When set to 20, it is a 1 in 20 chance this will happen.
</font>

### Overwrite subject

When you fill in the Overwrite subject field, that subject will be used to build the dynamic prompt around.
It is best, if you set the subject type to match the subject.
For example, set it to humanoid if you place a person in the override subject field.

This way, you can create unlimited variants of a subject.

Smart subject tries to determine what to and not to generate based on your subject.
Example, if your Overwrite subject is formed like this: Obese man wearing a kimono

It will then recognize the body type and not generate it.
It also recognizes the keyword wearing, and will not generate an outfit.
...
They will be automatically added during processing.

These can be used to add textual inversion and LoRA's to always apply.
They can also be used to add your models trigger words.
...
### Filter values
<font size="2">
You can put comma seperated values here, those will be ignored from any list processing.
For example, adding ""film grain, sepia"", will make these values not appear during generation.
...
Idea by redditor jonesaid.
        ...
        with gr.Tab("Workflow assist"):
            ...
            with gr.Row():
                promptvariantinsanitylevel = gr.Slider(
                    0,
                    10,
                    value=0,
                    step=1,
                    label="Prompt variant. Strength of variation of workflow prompt. 0 = no variance."
                )
            with gr.Row():
                gr.Markdown(
                    """
                    ...
                    With Workflow mode, you turn off the automatic generation of new prompts on 'generate'
                    and it will use the Workflow prompt field instead.
                    So you can work and finetune any fun prompts without turning of the script.

                    You can use One Button Prompt wildcards in the workflow prompt. For example -outfit- .

                    With the Prompt Variant, you can let One Button Prompt dynamically create small variance
                    in the workflow prompt. 0 means no effect.

                    Below here, you can generate a set of random prompts, and send them to the Workflow prompt field.
                    The generation of the prompt uses the settings in the Main tab.
                    </font>
                    """
                )
            with gr.Row():
        ...
        with gr.Tab("Advanced"):
            with gr.Row():
                with gr.Column(scale=1):
                    promptcompounderlevel = gr.Dropdown(promptcompounder, label="Prompt compounder", value="1")
            with gr.Row():
                with gr.Column(scale=1):
                    seperator = gr.Dropdown(seperatorlist, label="Prompt seperator", value="comma")
                with gr.Column(scale=2):
                    ANDtoggle = gr.Dropdown(ANDtogglemode, label="Prompt seperator mode", value="none")
            with gr.Row():
                gr.Markdown(
                    """
                    ### Prompt compounder
                    ...
                    <font size="2">
                    Normally, it creates a single random prompt. With prompt compounder,
                    it will generate multiple prompts and compound them together.
                    ...
                    This was originally a bug in the first release when using multiple batches,
                    now brought back as a feature.
                    Raised by redditor drone2222, to bring this back as a toggle,
                    since it did create interesting results. So here it is.
                    ...
                    "automatic" is entirely build around Latent Couple.
                    It will pass artists and the amount of people/animals/objects to generate
                    in the prompt automatically.
                    Set the prompt compounder equal to the amount of areas defined in Laten Couple.
        ...
        with gr.Tab("One Button Run and Upscale"):
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    gr.Markdown(
                        ...
                        Instead it will pick up all files in the \\upscale_me\\ folder and
                        upscale them with below settings.
                        </font>
                        """
                    )
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    size = gr.Dropdown(sizelist, label="Size to generate", value="all")
                    basesize = gr.Dropdown(basesizelist, label="base size", value="512")
                    with gr.Row(scale=1):
                ...
                with gr.Column(scale=1):
                    ...
                    with gr.Column(scale=1):
                        samplingmethod = gr.Dropdown(samplerlist, label="Sampler", value="all")
                        upscaler = gr.Dropdown(upscalerlist, label="hires upscaler", value="all")
            with gr.Row():
                gr.Markdown(
                    """
                    ### Quality Gate
                    ...
                    Once turned on, it will retry for n amount of times to get an image with the quality score.
                    If not, it will take the best image so far and continue or set it to gated to only take
                    matching or higher quality images.
            ...
            with gr.Row():
                ...
                qualitymode = gr.Dropdown(qualitymodelist, label="Mode of operation", value="highest", visible=False)
                qualitykeep = gr.Dropdown(qualitykeeplist, label="Images", value="keep used", visible=False)
            with gr.Row():
                gr.Markdown(
                    """
                    ### IMG2IMG upscale
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    img2imgbatch = gr.Slider(
                        1,
                        5,
                        value="1",
                        step=1,
                        label="Amount times to repeat upscaling with IMG2IMG (loopback)"
                    )
                    ...
                    img2imgdenoisestrengthmod = gr.Slider(
                        -1,
                        1,
                        value="-0.05",
                        step=0.01,
                        label="adjust denoise each img2img batch"
                    )
                with gr.Column(scale=1):
                    img2imgmodel = gr.Dropdown(
                        modellist,
                        label="img2img model to use",
                        value="currently selected model"
                    )
                    img2imgsamplingmethod = gr.Dropdown(img2imgsamplerlist, label="img2img sampler", value="all")
                    img2imgupscaler = gr.Dropdown(img2imgupscalerlist, label="img2img upscaler", value="all")
                with gr.Row():
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    usduredraw = gr.Dropdown(redraw_modes, label="Type", value="Linear", visible=False)
                with gr.Column(scale=1):
                    usduSeamsfix = gr.Dropdown(seams_fix_types, label="Seams fix", value="None", visible=False)
                    usdusdenoise = gr.Slider(
                        0,
                        1,
                        value="0.35",
                        step=0.01,
                        label="Seams denoise strenght",
                        visible=False
                    )
                    ...
                    usdusmaskblur = gr.Slider(
                        0,
                        64,
                        value="8",
                        step=1,
                        label="Seams Mask blur (offset pass only)",
                        visible=False
                    )
            with gr.Row():
                with gr.Column(scale=1):
                ...
                with gr.Column(scale=1):
                    controlnetmodel = gr.Textbox(
                        label="Controlnet tile model name",
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    extrasupscaler2visiblity = gr.Slider(
                        0,
                        1,
                        value="0.5",
                        step=0.05,
                        label="Upscaler 2 vis.",
                        visible=False
                    )

                with gr.Column(scale=1):
                    extrasupscaler2gfpgan = gr.Slider(0, 1, value="0", step=0.05, label="GFPGAN vis.", visible=False)
                    extrasupscaler2codeformer = gr.Slider(
                        0,
                        1,
                        value="0.15",
                        step=0.05,
                        label="CodeFormer vis.",
                        visible=False
                    )
                    extrasupscaler2codeformerweight = gr.Slider(
                        0,
                        1,
                        value="0.1",
                        step=0.05,
        ...
        startmain.click(
            ...
            inputs=[
                ...
                controlnetenabled,
                controlnetmodel,
                img2imgdenoisestrengthmod,
                enableextraupscale,
                controlnetblockymode,
                extrasupscaler1,
                extrasupscaler2,
                extrasupscaler2visiblity,
                extrasupscaler2gfpgan,
                extrasupscaler2codeformer,
                extrasupscaler2codeformerweight,
                extrasresize,
                onlyupscale,
                givensubject,
                smartsubject,
        ...
        def subjectsvalue(subject):
            ...
            return {chosengender: gr.update(visible=enable), }
        ...
        def subjectsvalueforsubtypeobject(subject):
            ...
            return {chosensubjectsubtypeobject: gr.update(visible=enable), }
        ...
        def subjectsvalueforsubtypehumanoid(subject):
            enable = (subject == "humanoid")
            return {chosensubjectsubtypehumanoid: gr.update(visible=enable), }

        subject.change(subjectsvalueforsubtypehumanoid, [subject], [chosensubjectsubtypehumanoid])

        # turn things on and off for subject subtype concept
        def subjectsvalueforsubtypeconcept(subject):
            enable = (subject == "concept")
            return {chosensubjectsubtypeconcept: gr.update(visible=enable), }
        ...
        for i in range(batches):
            if silentmode is False:
                ...
                if ANDtoggle == "automatic":
                    ...
                    match subject:
                        case "humanoid":
                            preppedprompt += f', {promptcompounderlevel} people'
                        case "landscape":
                            preppedprompt += ", landscape"
                        case "animal":
                            preppedprompt += f', {promptcompounderlevel} animals'
                        case "object":
                            preppedprompt += f', {promptcompounderlevel} objects'

                    # sneaky! If we are running on automatic,
                    # we don't want "artists" to show up during the rest of the prompt,
                    # so set it to none, but only temporary!
                    artist = "none"
