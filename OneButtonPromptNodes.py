...
from csv_reader import load_config_csv, re
...
if generatemanwoman or generatemanwomanrelation or generatefictionalcharacter or generatenonfictionalcharacter \
        or generatehumanoids or generatejob:
...
class OneButtonPrompt:
    ...
    def Comfy_OBP(
        ...
        generatedprompt = build_dynamic_prompt(
            insanitylevel,
            subject,
            artist,
            imagetype,
...
class SavePromptToFile:
    ...
    def saveprompttofile(self, positive_prompt, negative_prompt, filename_prefix):
        ...
        full_output_folder, filename_short, counter, subfolder, filename_prefix = \
        ...
        if positive_prompt.find("of a ") != -1:
            start_index = f'{positive_prompt.find("of a ")}{len("of a ")}'
        ...
        if filename_prefix == "":
            ...
            now = datetime.now()
            filenamecomplete = f'{now.strftime("%Y%m%d%H%M%S")}_{filename.replace(" ", "_").strip()}.txt'
        else:
            ...
            filenamecomplete = f'{filename_short}_{formatted_counter}_{filename.replace(" ", "_").strip()}.txt'
