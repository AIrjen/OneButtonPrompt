...
from call_txt2img import call_txt2img
...
def generateimages(
    ...
    seperator="comma",
    img2imgbatch="1",
    img2imgsamplingsteps="20",
    img2imgcfg="7",
    img2imgsamplingmethod="DPM++ SDE Karras",
    img2imgupscaler="R-ESRGAN 4x+",
    img2imgmodel="currently selected model",
    img2imgactivate=False,
    img2imgscale="2",
    img2imgpadding="64",
    img2imgdenoisestrength="0.3",
    ...
    controlnetmodel="",
    img2imgdenoisestrengthmod="-0.05",
    enableextraupscale=False,
    controlnetblockymode=False,
    extrasupscaler1="all",
    extrasupscaler2="all",
    ...
    increasestability=False,
    qualityhiresfix=False,
    qualitymode="highest",
    qualitykeep="keep used",
    basesize="512",
    promptvariantinsanitylevel=0
):
    ...
    while steps < loops:
        ...
        if steps > 0 and increasestability is True:
            print("")
            ...
            requests.post(url=f'{apiurl}/sdapi/v1/unload-checkpoint')
            print("model unloaded")

            requests.post(url=f'{apiurl}/sdapi/v1/reload-checkpoint')
            print("model reloaded")

        # build prompt
        if silentmode is True and workprompt == "":
            print("Trying to use provided workflow prompt, but is empty. Generating a random prompt instead.")

        if onlyupscale is False:  # only do txt2img when onlyupscale is False
            if silentmode is True and workprompt != "":
                randomprompt = createpromptvariant(workprompt, promptvariantinsanitylevel)
                print("Using provided workflow prompt")
                print(randomprompt)
            else:
                randomprompt = build_dynamic_prompt(
                    insanitylevel,
                    subject,
                    artist,
                    imagetype,
            ...
            now = datetime.now()
            filenamecomplete = f'{now.strftime("%Y%m%d%H%M%S")}_{filename.replace(" ", "_").strip()}'
            ...
            if originalmodel == "all":
                ...
                while "inpaint" in model:
                    model = random.choice(modellist)
                print(f"Going to run with model {model}")
            if originalmodel == "currently selected model":
                model = currentlyselectedmodel

            # set the model here
            option_payload = {"sd_model_checkpoint": model}
            requests.post(url=f'{apiurl}/sdapi/v1/options', json=option_payload)

            if originalsamplingmethod == "all":
                samplingmethod = random.choice(samplerlist)
                print(f"Going to run with sampling method {samplingmethod}")

            if originalupscaler == "all" and hiresfix is True:
                upscaler = random.choice(upscalerlist)
                print(f"Going to run with upscaler {upscaler}")

            # WebUI fix for PLMS and UniPC and hiresfix
            # PLMS/UniPC do not support hirefix so we just silently switch to DDIM
            if samplingmethod in ['PLMS', 'UniPC']:
                samplingmethod = 'DDIM'

            txt2img = call_txt2img(
                randomprompt,
                size,
                hiresfix,
                0,
        ...
        if img2imgactivate is False or continuewithnextpart is False:  # If we dont want to run, turn it off
            img2imgloops = 0
        else:
            # Check if there is any random value we have to choose or not
        ...
        while img2imgsteps < img2imgloops:
            ...
            # lower or increase the denoise strength for each batch
            ...
            # Sometimes, we are too quick to do another call, causing memory issues.
            # So we wait a bit to let the system settle done a bit.
        ...
        if enableextraupscale is True and continuewithnextpart is True:
        ...
        if continuewithnextpart is True:
...
def tryinterrupt(apiurl="http://127.0.0.1:7860"):
    requests.post(url=f'{apiurl}/sdapi/v1/interrupt')
