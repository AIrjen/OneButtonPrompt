...
from model_lists import get_upscalers
...
def call_txt2img(
    ...
    # upscaler
    enable_hr = upscale
    if debugmode == 1 or qualityhiresfix is True:
        enable_hr = "False"
    ...
    if upscaler != "automatic":
        ...
        elif "anime" in checkprompt or "cartoon" in checkprompt or "draw" in checkprompt or "vector" in checkprompt or \
                "cel shad" in checkprompt or "visual novel" in checkprompt:
    ...
    outputTXT2IMGtxtFull = f'{outputTXT2IMGtxtfolder}{filename}{outputTXT2IMGtxt}'
    ...
    while Runs < MaxRuns:
        ...
        outputTXT2IMGFull = f'{outputTXT2IMGfolder}{filenamefull}{outputTXT2IMGpng}'
        ...
        for i in r['images']:
            ...
            if qualitygate is True:
                # check if the file exists in the parent directory
                imagescorer_path = os.path.abspath(os.path.join(
                ...
                try:
                    ...
                    print(f'This image has scored: {score} out of {isGoodNumber}')
                    if score >= isGoodNumber or debugmode == 1:
                        foundgood = True
                        print("Yay its good! Keeping this result.")
                    else:
                        runstodo = MaxRuns - Runs - 1
                        print('Not a good result.')
                        print(f'Retrying for another {runstodo} times or until the image is good enough.')
    ...
    if foundgood is False and qualitymode != "highest":
        continuewithnextpart = False
        if imagethatiskept != "" and qualitykeep == "keep used":
            os.remove(imagethatiskept)

    # We have done everything, but if we want to run Hires fix from the quality gate,
    # we are going to have to do it again. But this time a little easier.
    # We do have the check wether we want to run hiresfix first
    if qualityhiresfix is True and upscale is False and continuewithnextpart is True:
        print("Quality Gate hires fix was enabled, but no hires fix settings were given.")
    if qualityhiresfix is True and upscale is True and continuewithnextpart is True:
        ...
        for i in range(4):
            ...
            if 'images' in r:
                ...
                if i == 3:
                    ...
                    raise ValueError("API has not been responding after several retries. Stopped processing.")
                print("")
                print("We haven't received an image from the API. Maybe something went wrong.")
                print("Will retry after waiting a bit.")
...
def setsize(ratio, basesize, originalsize):
    ...
    match ratio:
        ...
        case "ultraheight":
            ...
            height = "1280"
        case "wild":
            width = height = str(round((random.randint(0, 4) * 128) + (int(basesize) / 2)))
        case _:
            width = basesize
            height = basesize
