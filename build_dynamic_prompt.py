...
from csv_reader import (artist_category_csv_to_list, csv_to_list,
                        load_config_csv)
from random_functions import (chance_roll, common_dist, legendary_dist,
                              normal_dist, novel_dist, rare_dist,
                              uncommon_dist, unique_dist)
...
def build_dynamic_prompt(
    ...
    outfitprinttotallist = \
        objecttotallist + locationlist + colorlist + musicgenrelist + seasonlist + animallist + patternlist

    # build artists list
    artistlist = []
    # create artist list to use in the code, maybe based on category  or personal lists
    if artists != "all" and artists != "none" and artists.startswith("personal_artists") is False \
            and artists.startswith("personal artists") is False:
        artistlist = artist_category_csv_to_list("artists_and_category", artists)
    elif artists.startswith("personal_artists") is True or artists.startswith("personal artists") is True:
    ...
    generateobject = \
        generatevehicle or generateobject or generatefood or generatebuilding or generatespace or generateflora
    ...
    generatehumanoid = generatefictionalcharacter or generatenonfictionalcharacter or generatehumanoids or \
        generatemanwoman or generatejob or generatemanwomanrelation or generatefirstnames
    ...
    # Smart subject logic
    if givensubject != "" and smartsubject is True:
        ...
        if foundinlist is True or keywordsinstring is True:
        ...
        if foundinlist is True or foundinlist2 is True:
        ...
        keywordsinstring = any(word.lower() in givensubject.lower() for word in keywordslist)
        if foundinlist is True or foundinlist2 is True or keywordsinstring is True:
    ...
    while compoundcounter < promptstocompound:
        ...
        if templatemode is True:
            ...
            templateprompts = [
                templateprompt[0] for templateprompt in templatelist if
                (templateprompt[1] == targettemplateenvironment or targettemplateenvironment == "all") and
                (templateprompt[2] == templateenvironmentsources or templateenvironmentsources == "all") and
                (templateprompt[3] == forcesubject or forcesubject == "all")
            ]
            templatepromptcreator = [
                templateprompt[1] for templateprompt in templatelist if
                (templateprompt[1] == targettemplateenvironment or targettemplateenvironment == "all") and
                (templateprompt[2] == templateenvironmentsources or templateenvironmentsources == "all") and
                (templateprompt[3] == forcesubject or forcesubject == "all")
            ]
            templatesubjects = [
                templateprompt[4] for templateprompt in templatelist if
                (templateprompt[1] == targettemplateenvironment or targettemplateenvironment == "all") and
                (templateprompt[2] == templateenvironmentsources or templateenvironmentsources == "all") and
                (templateprompt[3] == forcesubject or forcesubject == "all")
            ]
        ...
        for i in range(custominputprefixrepeats):
            if chance_roll(insanitylevel, custominputprefixchance) and generatecustominputprefix is True:
                completeprompt += f'{random.choice(custominputprefixlist)}, '
        ...
        # 0 object, 1 animal, 2 animal as human, 3 ManWoman, 4 Job
        # 5 fictional, 6 non fictional, 7 humanoid, 8 landscape, 9 event
        ...
        if chance_roll(insanitylevel, artistsatbackchance) and onlyartists is False:
            artistsplacement = "back"

        if artists != "none" and artistsplacement == "front" and generateartist is True:
            ...
            if modeselector < 5 and end - step >= 2:
                ...
                if advancedprompting is False:
                    artistmode = "normal"
                if (artistmode in ["hybrid", "switching"] and end - step == 1):
                    artistmode = "normal"

            if onlyartists is True and artistmode == "enhancing":
                artistmode = "normal"
            # if there are not enough artists in the list, then just go normal
            if len(artistlist) < 3:
                artistmode = "normal"
            if onlyartists is True and step == end:
                step = step - 1
            ...
            while step < end:
                ...
                completeprompt += "-artist-"
                if isweighted == 1:
                    completeprompt += f':{1 + (random.randint(-3, 3) / 10)})'
            ...
            if artistmode in ["stopping"]:
                completeprompt += f'::{random.randint(1, 19)}'

            if artistmode in ["switching", "adding"]:
                completeprompt += f':{random.randint(1, 18)}'
            if artistmode in ["hybrid", "stopping", "adding", "switching"]:
                completeprompt += "] "

            if onlyartists is True:
                ...
                print(f'only generated these artists:{completeprompt}')
        ...
        if giventypeofimage == "" and generatetype is True:
            ...
            if othertype == 1:
                ...
                completeprompt += ", "
        elif generatetype is True:
            othertype = 1
            completeprompt += f'{giventypeofimage} of a '

        # here we can do some other stuff to spice things up
        if chance_roll(insanitylevel, minilocationadditionchance) and generateminilocationaddition is True:
            completeprompt += " -minilocationaddition-, "

        if chance_roll(insanitylevel, artmovementprefixchance) and generateartmovement is True:
            generateartmovement = False
            completeprompt += " -artmovement-, "

        if chance_roll(insanitylevel, minivomitprefix1chance) and generateminivomit is True:
            completeprompt += " -minivomit-, "

        if chance_roll(insanitylevel, minivomitprefix2chance) and generateminivomit is True:
            completeprompt += " -minivomit-, "

        # start shot size
        if mainchooser in ["object", "animal", "humanoid", "concept"] and othertype == 0 and "portrait" \
                not in completeprompt and generateshot is True and chance_roll(insanitylevel, shotsizechance):
            completeprompt += "-shotsize- of a "
        elif "portrait" in completeprompt and generateshot is True:
            completeprompt += " ,close up of a "
        elif mainchooser in ["landscape"] and generateshot is True:
            completeprompt += " landscape of a "
        elif generateshot is True:
            completeprompt += ", "

        # start subject building
        if generatesubject is True:
            # start with descriptive qualities
            # Common to have 1 description, uncommon to have 2
            if chance_roll(insanitylevel, subjectdescriptor1chance) and generatedescriptors is True:
                completeprompt += "-descriptor- "

            if chance_roll(insanitylevel, subjectdescriptor2chance) and generatedescriptors is True:
                completeprompt += "-descriptor- "
            ...
            if subjectchooser in [
                ...
                "manwomanrelation",
                "firstname"
            ] and chance_roll(insanitylevel, subjectbodytypechance) and generatebodytype is True:
            ...
            if subjectchooser in [
                ...
                "manwomanrelation",
                "firstname"
            ] and chance_roll(insanitylevel, subjectculturechance) and generatedescriptors is True:
            ...
            if mainchooser == "object":
                ...
                if givensubject == "":
                    if rare_dist(insanitylevel) and advancedprompting is True:
                    ...
                    completeprompt += f'{chosenobjectwildcard} '

                    if hybridorswap == "hybrid":
                        if uncommon_dist(insanitylevel):
                            completeprompt += f'|{random.choice(objectwildcardlist)}] '
                        else:
                            completeprompt += f'|{chosenobjectwildcard} ] '
                    if hybridorswap == "swap":
                        if uncommon_dist(insanitylevel):
                            completeprompt += f':{random.choice(objectwildcardlist)}:{random.randint(1, 5)}] '
                        else:
                            completeprompt += f':{chosenobjectwildcard} :{random.randint(1, 5)}] '
                else:
                    completeprompt += f' {givensubject} '
            ...
            if mainchooser == "animal":
                ...
                if givensubject == "":
                    ...
                    if unique_dist(insanitylevel) and generateanimaladdition is True:
                    ...
                    if hybridorswap == "hybrid":
                        if uncommon_dist(insanitylevel):
                            completeprompt += f'|{random.choice(hybridlist)}] '
                        else:
                            completeprompt += "| -animal- ] "
                    if hybridorswap == "swap":
                        if uncommon_dist(insanitylevel):
                            completeprompt += f':{random.choice(hybridlist)}:{random.randint(1, 5)}] '
                        else:
                            completeprompt += f':-animal-:{random.randint(1, 5)}] '
                else:
                    completeprompt += f' {givensubject} '
            ...
            if mainchooser == "humanoid":
                ...
                if givensubject == "":
                    ...
                    if subjectchooser == "fictional":
                        ...
                        if hybridorswap == "hybrid":
                            completeprompt += f'|{random.choice(hybridhumanlist)} ] '
                        if hybridorswap == "swap":
                            completeprompt += f':{random.choice(hybridhumanlist)}:{random.randint(1, 5)}] '
                        hybridorswap = ""
                    ...
                    if subjectchooser == "firstname":
                        ...
                        if hybridorswap == "hybrid":
                            completeprompt += '|-firstname-] '
                        if hybridorswap == "swap":
                            completeprompt += f':-firstname-:{random.randint(1, 5)}] '
                        hybridorswap = ""
                else:
            ...
            if subjectchooser == "landscape":
                ...
                if givensubject == "":
                    ...
                    if hybridorswap == "hybrid":
                        completeprompt += '|-location-] '
                    if hybridorswap == "swap":
                        completeprompt += f':-location-:{random.randint(1, 5)}] '
                else:
                    completeprompt += f' {givensubject} '
            ...
            if mainchooser == "concept":
                ...
                if givensubject == "":
                    if subjectchooser == "event":
                        completeprompt += f' "{random.choice(eventlist)}" '
        ...
        for i in range(objectadditionsrepeats):
            if mainchooser == "object" and chance_roll(insanitylevel, objectadditionschance) \
                    and generateobjectaddition is True:
        ...
        if subjectchooser in [
            ...
            "manwomanrelation",
            "firstname"
        ] and chance_roll(insanitylevel, humanadditionchance) and generatehumanaddition is True:
            humanspecial = 1
        ...
        if chance_roll(insanitylevel, overalladditionchance) and generateoveralladdition is True:
        ...
        if subjectchooser in [
            ...
            "manwomanrelation",
            "firstname"
        ] and chance_roll(insanitylevel, emojichance) and generateemoji is True:
        ...
        if subjectchooser in [
            ...
            "manwomanrelation",
            "firstname"
        ] and chance_roll(insanitylevel, joboractivitychance) and humanspecial != 1 and generatesubject is True:
            joboractivitylist = [joblist, humanactivitylist]
            completeprompt += f'{random.choice(random.choice(joboractivitylist))}, '

        # custom mid list
        for i in range(custominputmidrepeats):
            if chance_roll(insanitylevel, custominputmidchance) and generatecustominputmid is True:
                completeprompt += f'{random.choice(custominputmidlist)}, '

        # add in some more mini vomits
        if chance_roll(insanitylevel, minivomitmidchance) and generateminivomit is True:
            completeprompt += " -minivomit-, "
        ...
        if subjectchooser in [
            ...
            "manwomanrelation",
            "firstname"
        ] and chance_roll(insanitylevel, outfitchance) and generateoutfit is True and humanspecial != 1:
            completeprompt += f', wearing {random.choice(buildoutfitlist)}, '
        ...
        if subjectchooser in [
            ...
            "manwomanrelation",
            "firstname"
        ] and chance_roll(insanitylevel, posechance) and humanspecial != 1 and generatepose is True:
            completeprompt += f'{random.choice(poselist)}, '
        ...
        if subjectchooser in [
            ...
            "manwomanrelation",
            "firstname"
        ] and chance_roll(insanitylevel, hairchance) and generatehairstyle is True:
            completeprompt += f'{random.choice(buildhairlist)}, '
        ...
        if subjectchooser in [
            ...
            "manwomanrelation",
            "firstname"
        ] and chance_roll(insanitylevel, accessorychance) and generateaccessorie is True \
                and generateaccessories is True:
            completeprompt += f'{random.choice(buildaccessorielist)}, '

        if chance_roll(insanitylevel, humanoidinsideshotchance) and subjectchooser not in ["landscape", "concept"] \
                and generateinsideshot is True:
            insideshot = 1
            completeprompt += f'{random.choice(insideshotlist)}, '

        if subjectchooser not in ["landscape", "concept"] and humanspecial != 1 and insideshot == 0 \
                and chance_roll(insanitylevel, humanoidbackgroundchance) and generatebackground is True:
            completeprompt += f'{random.choice(backgroundtypelist)}, '

        # minilocation bit
        if subjectchooser in ["landscape"] and chance_roll(insanitylevel, landscapeminilocationchance) \
                and generateminilocationaddition is True:
            completeprompt += " -minilocationaddition-, "

        if chance_roll(insanitylevel, generalminilocationchance) and generateminilocationaddition is True:
            completeprompt += " -minilocationaddition-, "
        ...
        if chance_roll(insanitylevel, timperiodchance) or subjectchooser == "landscape":
            if generatetimeperiod is True:
                completeprompt += "-timeperiod-, "

        if mainchooser not in ["landscape"] and chance_roll(insanitylevel, focuschance) and generatefocus is True:
            completeprompt += "-focus-, "

        # others
        if chance_roll(insanitylevel, directionchance) and generatedirection is True:
            completeprompt += "-direction-, "

        if chance_roll(insanitylevel, moodchance) and generatemood is True:
            completeprompt += "-mood-, "

        # add in some more mini vomits
        if chance_roll(insanitylevel, minivomitsuffixchance) and generateminivomit is True:
            completeprompt += " -minivomit-, "

        if chance_roll(insanitylevel, artmovementchance) and generateartmovement is True:
            completeprompt += "-artmovement-, "

        if chance_roll(insanitylevel, lightingchance) and generatelighting is True:
            completeprompt += "-lighting-, "
        ...
        if chance_roll(insanitylevel, photoadditionchance) and isphoto == 1 and generatephotoaddition is True:
            completeprompt += f'{random.choice(photoadditionlist)}, '

        if isphoto == 1 and generatecamera is True:
            completeprompt += "-camera-, "

        if chance_roll(insanitylevel, lenschance) or isphoto == 1:
            if generatelens is True:
                completeprompt += "-lens-, "

        if chance_roll(insanitylevel, colorschemechance) and generatecolorscheme is True:
            completeprompt += "-colorscheme-, "

        # vomit some cool/wierd things into the prompt
        if chance_roll(insanitylevel, vomit1chance) and generatevomit is True:
            completeprompt += "-vomit-, "
        ...
        if chance_roll(insanitylevel, greatworkchance) and generategreatwork is True:
            completeprompt += " in the style of -greatwork-, "

        # adding a poemline. But this should happen only very rarely.
        if chance_roll(insanitylevel, poemlinechance) and generatepoemline is True:
            completeprompt += " \"-poemline-\", "

        # adding a songline. But this should happen only very rarely.
        if chance_roll(insanitylevel, songlinechance) and generatesongline is True:
            completeprompt += " \"-songline-\", "

        # everyone loves the adding quality. The better models don't need this, but lets add it anyway
        if chance_roll(insanitylevel, quality1chance) and generatequality is True:
            completeprompt += "-quality-, "
        ...
        # custom style list
        if chance_roll(insanitylevel, customstyle1chance) and generatestyle is True:
        ...
        for i in range(custominputsuffixrepeats):
            if chance_roll(insanitylevel, custominputsuffixchance) and generatecustominputsuffix is True:
                completeprompt += f'{random.choice(custominputsuffixlist)}, '

        if artistmode in ["enhancing"]:
            completeprompt += f'::{random.randint(1, 17)}] '

        if artists != "none" and artistsplacement == "back" and generateartist is True:
            completeprompt += ", "
            # take 1-3 artists, weighted to 1-2
            step = random.randint(0, 1)
            end = random.randint(1, insanitylevel3)
    ...
    if len(completeprompt) > 325 and matches == []:
        ...
        completeprompt = completeprompt.replace("-objectstrengthend-", f':{strenght})')
    else:
        completeprompt = completeprompt.replace("-objectstrengthstart-", "")
...
def createpromptvariant(
    ...
    objecttotallist = objectlist + buildinglist + vehiclelist + foodlist + spacelist + floralist + containerlist
    outfitprinttotallist = objecttotallist + locationlist + colorlist + musicgenrelist + seasonlist + animallist \
        + patternlist
    ...
    if insanitylevel != 0:
        ...
        while originalprompt == prompt and runs != maxamountofruns:
            for combination in combinations_list:
                comb = combination.lower()
                combination = f' {combination} '

                # some rare changes if needed
                if comb in [x.lower() for x in humanlist] and chance_roll(insanitylevel, "rare"):
                    prompt = prompt.replace(combination, " -human- ")
                if comb in [x.lower() for x in objecttotallist] and chance_roll(insanitylevel, "rare"):
                    prompt = prompt.replace(combination, " -objecttotal- ")
                if comb in [x.lower() for x in artistlist] and chance_roll(insanitylevel, "rare"):
                    prompt = prompt.replace(combination, " -artist- ")
                if comb in [x.lower() for x in colorlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -color- ")
                if comb in [x.lower() for x in animallist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -animal- ")
                if comb in [x.lower() for x in objectlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -object- ")
                if comb in [x.lower() for x in fictionallist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -fictional- ")
                if comb in [x.lower() for x in nonfictionallist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -nonfictional- ")
                if comb in [x.lower() for x in buildinglist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -building- ")
                if comb in [x.lower() for x in vehiclelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -vehicle- ")
                if comb in [x.lower() for x in outfitlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -outfit- ")
                if comb in [x.lower() for x in locationlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -location- ")
                if comb in [x.lower() for x in accessorielist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -accessory- ")
                if comb in [x.lower() for x in artmovementlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -artmovement- ")
                if comb in [x.lower() for x in bodytypelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -bodytype- ")
                if comb in [x.lower() for x in cameralist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -camera- ")
                if comb in [x.lower() for x in colorschemelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -colorscheme- ")
                if comb in [x.lower() for x in culturelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -culture- ")
                if comb in [x.lower() for x in descriptorlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -descriptor- ")
                if comb in [x.lower() for x in directionlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -direction- ")
                if comb in [x.lower() for x in emojilist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -emoji- ")
                if comb in [x.lower() for x in eventlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -event- ")
                if comb in [x.lower() for x in focuslist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -focus- ")
                if comb in [x.lower() for x in greatworklist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -greatwork- ")
                if comb in [x.lower() for x in haircolorlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -haircolor- ")
                if comb in [x.lower() for x in hairstylelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -hairstyle- ")
                if comb in [x.lower() for x in directionlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -direction- ")
                if comb in [x.lower() for x in humanoidlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -humanoid- ")
                if comb in [x.lower() for x in joblist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -job- ")
                if comb in [x.lower() for x in lenslist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -lens- ")
                if comb in [x.lower() for x in lightinglist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -lighting- ")
                if comb in [x.lower() for x in malefemalelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -malefemale- ")
                if comb in [x.lower() for x in manwomanlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -manwoman- ")
                if comb in [x.lower() for x in moodlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -mood- ")
                if comb in [x.lower() for x in othertypelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -othertype- ")
                if comb in [x.lower() for x in poselist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -pose- ")
                if comb in [x.lower() for x in qualitylist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -quality- ")
                if comb in [x.lower() for x in shotsizelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -shotsize- ")
                if comb in [x.lower() for x in timeperiodlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -timeperiod- ")
                if comb in [x.lower() for x in vomitlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -vomit- ")
                if comb in [x.lower() for x in foodlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -food- ")
                if comb in [x.lower() for x in genderdescriptionlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -genderdescription- ")
                if comb in [x.lower() for x in minilocationlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -minilocation- ")
                if comb in [x.lower() for x in minioutfitlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -minioutfit- ")
                if comb in [x.lower() for x in lenslist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -lens- ")
                if comb in [x.lower() for x in seasonlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -season- ")
                if comb in [x.lower() for x in imagetypequalitylist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -imagetypequality- ")
                if comb in [x.lower() for x in rpgclasslist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -rpgclass- ")
                if comb in [x.lower() for x in brandlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -brand- ")
                if comb in [x.lower() for x in spacelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -space- ")
                if comb in [x.lower() for x in poemlinelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -poemline- ")
                if comb in [x.lower() for x in songlinelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -songline- ")
                if comb in [x.lower() for x in musicgenrelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -musicgenre- ")
                if comb in [x.lower() for x in manwomanrelationlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -manwomanrelation- ")
                if comb in [x.lower() for x in waterlocationlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -waterlocation- ")
                if comb in [x.lower() for x in containerlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -container- ")
                if comb in [x.lower() for x in firstnamelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -firstname- ")
                if comb in [x.lower() for x in floralist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -flora- ")
                if comb in [x.lower() for x in printlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -print- ")
                if comb in [x.lower() for x in miniactivitylist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -miniactivity- ")
                if comb in [x.lower() for x in patternlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -pattern- ")
                if comb in [x.lower() for x in chairlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -chair- ")
                if comb in [x.lower() for x in fantasyartistlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -artistfantasy- ")
                if comb in [x.lower() for x in popularartistlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -artistpopular- ")
                if comb in [x.lower() for x in romanticismartistlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -artistromanticism- ")
                if comb in [x.lower() for x in photographyartistlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -artistphotography- ")
                if comb in [x.lower() for x in stylestiloralist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -styletilora- ")
                if comb in [x.lower() for x in waterlocationlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -waterlocation- ")
                if comb in [x.lower() for x in imagetypemodelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -imagetypemodes- ")
                if comb in [x.lower() for x in buildaccessorielist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -buildaccessorie- ")
                if comb in [x.lower() for x in buildoutfitlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -buildoutfit- ")
                if comb in [x.lower() for x in buildhairlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -buildhair- ")
                if comb in [x.lower() for x in insideshotlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -insideshots- ")
                if comb in [x.lower() for x in photoadditionlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -photoadditions- ")
                if comb in [x.lower() for x in backgroundtypelist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -backgroundtypes- ")
                if comb in [x.lower() for x in customsubjectslist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -custom_subjects- ")
                if comb in [x.lower() for x in custominputsuffixlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -custom_input_suffix- ")
                if comb in [x.lower() for x in custominputmidlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -custom_input_mid- ")
                if comb in [x.lower() for x in custominputprefixlist] and chance_roll(insanitylevel, "uncommon"):
                    prompt = prompt.replace(combination, " -custom_input_prefix- ")
    ...
    while (
        ...
        "-styletilora-" in completeprompt or
        "-waterlocation-" in completeprompt or
        "-imagetypemodes-" in completeprompt or
        "-buildaccessorie-" in completeprompt or
        "-buildoutfit-" in completeprompt or
        "-buildhair-" in completeprompt or
        "-insideshots-" in completeprompt or
        "-photoadditions-" in completeprompt or
        "-backgroundtypes-" in completeprompt or
        "-custom_subjects-" in completeprompt or
        "-custom_input_suffix-" in completeprompt or
        "-custom_input_mid-" in completeprompt or
        "-custom_input_prefix-" in completeprompt
    ):
        allwildcardslistnohybrid = [
            "-color-",
            "-object-",
        ...
        allwildcardslistnohybridlists = [
            ...
            backgroundtypelist,
            customsubjectslist,
            custominputsuffixlist,
            custominputmidlist,
            custominputprefixlist
        ]
...
def replacewildcard(completeprompt, insanitylevel, wildcard, listname, activatehybridorswap, advancedprompting):
    if len(listname) == 0:
        ...
        while wildcard in completeprompt:
            if unique_dist(insanitylevel) and activatehybridorswap is True and len(listname) > 2 \
                    and advancedprompting is True:
                hybridorswaplist = ["hybrid", "swap"]
                hybridorswap = random.choice(hybridorswaplist)
                replacementvalue = random.choice(listname)
                listname.remove(replacementvalue)
                hybridorswapreplacementvalue = f'[{replacementvalue}'
                ...
                if hybridorswap == "swap":
                    replacementvalue = random.choice(listname)
                    listname.remove(replacementvalue)
                    hybridorswapreplacementvalue += f':{replacementvalue}:{random.randint(1, 20)}]'
            ...
            if bool(listname):
                ...
                listname.remove(replacementvalue)
                if wildcard == "-artist-":
                    replacementvalue = f'art by {replacementvalue}'
            else:
                replacementvalue = ""
